...
import gov.nist.secauto.swid.client.service.HTTPServiceImpl;
import gov.nist.secauto.swid.client.service.TagType;
...
public class Application {
  ...
  protected CommandLine parseCLI(String[] args) throws ParseException {
    ...
    Option keystorePwd
        = Option.builder().longOpt(OPTION_KEYSTORE_PWD).desc("the client keystore password").hasArg().build();
    options.addOption(keystorePwd);

    Option seed = Option.builder().longOpt(OPTION_SEED).desc("the client seed").hasArg().build();
    options.addOption(seed);

    Option tagType = Option.builder().longOpt(OPTION_TAGTYPE).desc("the swid tag type - " + TagType.displayTagTypes())
        .hasArg().build();
    ...
    for (String s : args) {
      if (s.equals("-h") || s.equals("--help")) {
        String header
            = "Post Software Identification(SWID) tags to National Vulnerability Database(NVD) repository\n\n";
  ...
  private int run(String[] args) throws ParseException {
    ...
    try {
      ...
      swidService.postSwid(clientCertificatePath, clientCertificatePassword, passwordSeed, tagFileNames, swidAction,
          type);
