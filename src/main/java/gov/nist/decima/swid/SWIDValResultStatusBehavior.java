...
public class SWIDValResultStatusBehavior implements ResultStatusBehavior {
  private static final String TAG_TYPES_QNAME = "{" + XMLConstants.REQUIREMENTS_SWID_EXTENSION_NS
      + "}tag-type";
  private static final String SCOPE_QNAME = "{" + XMLConstants.REQUIREMENTS_SWID_EXTENSION_NS
      + "}scope";
  ...
  public boolean isInScope(Requirement requirement) {
    ...
    boolean matchTagType = tagTypes == null || tagTypes.isEmpty() || tagTypes.contains("all")
        || tagTypes.contains(tagType.getName());
    boolean matchScope = scopeValues == null || scopeValues.isEmpty() || scopeValues.contains("all")
